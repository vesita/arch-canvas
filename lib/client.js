...
window.__ModuleLoader__.load({
    ...
    factory: (require) => {
        ...
        function mountConfigCard(ctx) {
            // `configForms` 由 `@deepseek-ai/dsh-client-ui-settings` 提供，是**可选**服务：
            // 缺它的部署（没装设置 UI / 旧构建）只该少一张配置卡，画布照常。所以走 `ctx.get`
            // （与下面 uiWorkspace 同一个范式）—— 列进 `exports.inject` 而部署里缺席时，
            // 客户端 runner 是 **park**（不是给 undefined），整个浏览器半边永不 apply，连画布一起没。
            var forms = undefined;
            try {
                forms = ctx && typeof ctx.get === 'function' ? ctx.get('configForms') : undefined;
            }
            catch (e) {
                forms = undefined;
            }
             ...
             */
            function pickDirectoryWith(workspace) {
                var bridge = typeof globalThis !== 'undefined' ? globalThis.__DSH_DIRECTORY_PICKER__ : undefined;
                if (bridge && typeof bridge.pick === 'function')
                    return bridge.pick();
                return workspace.pickDirectory();
            }
            /** 有没有可用的系统选择器：preload 桥，或宿主里的 native 能力入口。 */
            function hasDirectoryPicker(workspace) {
                var bridge = typeof globalThis !== 'undefined' ? globalThis.__DSH_DIRECTORY_PICKER__ : undefined;
                if (bridge && typeof bridge.pick === 'function')
                    return true;
            ...
            function ArchConfigCard(props) {
                ...
                function choose() {
                    ...
                    return Promise.resolve()
                        ...
                        if (typeof path === 'string' && path.length > 0)
                            props.edit(DATA_DIR_FIELD, path);
                        else
                            notice[1](null);
                    }, function (error) {
                ...
                }), React.createElement('div', { style: cardStyles.row }, React.createElement('button', {
                    type: 'button',
                    style: cardStyles.button,
                    disabled: busy[0] || locked,
                    onClick: choose,
                }, busy[0] ? '正在等待选择…' : '选择目录…'), React.createElement('span', { style: cardStyles.hint }, '打开系统文件管理器挑一个目录；选中只是填进上面的输入框，仍要点「保存」才写入。')), notice[0] !== null
        ...
        exports.apply = apply;
        // 界面本体依赖 slots（注册主窗口子页）。**只列真正的硬依赖**：
        //  - 不列 timer：没有它界面靠原生定时器回退（见 runtime.ts 的 ctxTimeout），列了而部署里没有就永远 park；
        //  - 不列 configForms：那是可选服务（设置 UI），缺席时只少一张配置卡，走 `ctx.get('configForms')`。
