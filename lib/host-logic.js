...
function keywordIsNode(line) {
    ...
    if (rest !== '' && ID_RE.test(rest.charAt(0)))
        return true;
    if (/^(\[|\(|\{|>|:::|@\{)/.test(rest))
        return true;
    var t = rest.replace(/^\s+/, '');
    if (t === '')
        return true; // 裸 `direction` / `link`：Mermaid 认它是一个孤立节点
...
function claimNodeId(owners, raw, warn) {
    ...
    if (existing === raw)
        return base;
    var probe = 2;
    while (owners[base + '_' + probe] !== undefined)
        probe++;
    var id = base + '_' + probe;
    owners[id] = raw;
...
function unquoteInner(text) {
    ...
    var a = 0;
    var b = s.length;
    while (a < b && /\s/.test(s.charAt(a)))
        a++;
    while (b > a && /\s/.test(s.charAt(b - 1)))
        b--;
    if (b - a >= 2 && s.charAt(a) === '"' && s.charAt(b - 1) === '"')
        return s.slice(a + 1, b - 1);
    return null;
}
    ...
    while (j < n && ID_RE.test(line.charAt(j))) {
        ...
        if (j > start) {
            ...
            for (var st = 0; st < ID_STOPS.length; st++) {
                if (line.slice(j, j + ID_STOPS[st].length) === ID_STOPS[st]) {
                    stops = true;
                    break;
                }
            }
...
function readRefAnnotations(line, ref, warn) {
    ...
    for (;;) {
        var p = k;
        while (p < n && /\s/.test(line.charAt(p)))
            p++;
        if (line.slice(p, p + 3) === ':::') {
            var cEnd = p + 3;
            while (cEnd < n && /[\w-]/.test(line.charAt(cEnd)))
                cEnd++;
            if (cEnd === p + 3)
                break; // `:::` 后面什么都没有：不认，交给主循环去失败
            k = cEnd;
            continue;
        }
        if (line.slice(p, p + 2) === '@{') {
            ...
            if (sh) {
                ...
                if (mapped)
                    ...
                    warn('节点 ' + ref.id + ' 的 `@{ shape: ' + sh[1] + ' }` 没能对应到已知形状，注解已丢弃（节点本身保留）：' + line);
            }
            var lb = /label\s*:\s*"((?:[^"\\]|\\.)*)"/.exec(body) || /label\s*:\s*'((?:[^'\\]|\\.)*)'/.exec(body);
            if (lb)
                ref.label = unquote('"' + lb[1].replace(/\\"/g, '#quot;').replace(/\\'/g, "'").replace(/\\n/g, '<br/>') + '"');
            else if (/label\s*:/.test(body))
                warn('节点 ' + ref.id + ' 的 `@{ … }` 里 label 没能解析，注解已丢弃（节点本身保留）：' + line);
            k = close + 1;
            continue;
        }
        break;
...
function parseMermaid(text) {
    ...
    for (var li = 0; li < raw.length; li++) {
        ...
        if (/^(flowchart|graph)\b/.test(line)) {
            ...
            if (dm) {
                if (dm[1])
                    doc.direction = dm[1].trim() === 'TB' ? 'TD' : dm[1].trim();
                continue;
            }
        }
        if (SUBGRAPH_RE.test(line)) {
            ...
            if (!badDecl) {
                ...
                if (glabel == null || String(glabel).trim() === '')
                    glabel = gid;
        ...
        if (DIRECTIVE_RE.test(line)) {
            if (!(AMBIGUOUS_KEYWORD.test(line) && keywordIsNode(line))) {
                doc.extras.push(line);
                continue;
            }
        }
    ...
    for (var ok in idOwners)
        rawOwned[idOwners[ok]] = true;
...
function serializeDoc(doc) {
    ...
    for (var lk = 0; lk < seq.length; lk++) {
        if (seq[lk].link)
            out.push('%% @link ' + cleanId(seq[lk].id) + ' ' + q(seq[lk].link));
    }
    ...
    for (var ft = 0; ft < seq.length; ft++) {
        ...
        for (var fr = 0; fr < frefs.length; fr++) {
            if (frefs[fr])
                out.push('%% @file ' + cleanId(seq[ft].id) + ' ' + qRef(frefs[fr]));
        }
    }
...
function stableJson(v) {
    ...
    if (v === null || typeof v !== 'object') {
        ...
        if (typeof v === 'number' && !isFinite(v))
            return '"\\u0000nonfinite:' + String(v) + '"';
        return JSON.stringify(v);
    }
    if (Array.isArray(v)) {
        var parts = [];
        for (var i = 0; i < v.length; i++)
            parts.push(stableJson(v[i]));
        return '[' + parts.join(',') + ']';
    }
    var ks = Object.keys(v).sort();
    var out = [];
    for (var k = 0; k < ks.length; k++)
        out.push(JSON.stringify(ks[k]) + ':' + stableJson(v[ks[k]]));
    return '{' + out.join(',') + '}';
}
/**
...
function roundTripNorm(m) {
    ...
    var out = {};
    for (var k in m) {
        if (RUNTIME_ONLY_FIELDS.indexOf(k) >= 0)
            continue;
        if (m[k] === undefined)
            continue;
        out[k] = m[k];
    ...
    if (Array.isArray(out.nodes)) {
        out.nodes = out.nodes.map(function (n) {
            ...
            for (var k2 in n) {
                if (n[k2] === undefined)
                    continue;
                if (OUT_OF_BAND_NODE_FIELDS.indexOf(k2) >= 0)
                    continue;
                c[k2] = n[k2];
            }
            if (typeof c.x === 'number' && isFinite(c.x))
                c.x = Math.round(c.x);
            if (typeof c.y === 'number' && isFinite(c.y))
                c.y = Math.round(c.y);
            ...
            if (c.label === null || c.label === undefined || c.label === '')
                c.label = c.id;
            return c;
        });
    }
    ...
    if (Array.isArray(out.nodes)) {
        out.nodes.sort(function (a, b) { return String(a && a.id) < String(b && b.id) ? -1 : String(a && a.id) > String(b && b.id) ? 1 : 0; });
    }
    if (Array.isArray(out.edges)) {
        out.edges = out.edges.map(function (e) {
            var c = {};
            for (var k3 in e) {
                if (e[k3] === undefined)
                    continue;
                if (OUT_OF_BAND_EDGE_FIELDS.indexOf(k3) >= 0)
                    continue;
                c[k3] = e[k3];
            }
            return c;
        });
        out.edges.sort(function (a, b) { return edgeSortKey(a) < edgeSortKey(b) ? -1 : edgeSortKey(a) > edgeSortKey(b) ? 1 : 0; });
    }
    if (Array.isArray(out.groups)) {
        out.groups = out.groups.map(function (g) {
            var c = {};
            for (var k4 in g) {
                if (g[k4] === undefined)
                    continue;
        ...
        out.groups.sort(function (a, b) { return String(a && a.id) < String(b && b.id) ? -1 : String(a && a.id) > String(b && b.id) ? 1 : 0; });
    }
...
function roundTripFieldDiff(a, b) {
    ...
    for (var k in A) {
        seen[k] = true;
        if (stableJson(A[k]) !== stableJson(B[k]))
            out.push(k);
    }
    for (var k2 in B)
        if (!seen[k2])
...
function roundTripBadIds(mine, theirs, keyOf) {
    ...
    for (var i = 0; i < n; i++) {
        if (stableJson(a[i]) === stableJson(b[i]))
            continue;
        var x = a[i] || b[i] || {};
...
function roundTripDiff(doc) {
    ...
    for (var k in mine) {
        seen[k] = true;
        if (stableJson(mine[k]) !== stableJson(theirs[k]))
            out.push(k);
    }
...
function roundTripDetail(doc) {
    ...
    try {
        ...
        return { fields: diff, detail: '解析回读失败' };
    }
    var mine = roundTripNorm(doc);
    var theirs = roundTripNorm(back);
    var detail = {};
...
var ARROW_SET = {};
for (var arrowSetIdx = 0; arrowSetIdx < ARROWS.length; arrowSetIdx++)
    ARROW_SET[ARROWS[arrowSetIdx]] = 1;
...
async function persist(policy, site, expectFile) {
    ...
    try {
        ...
        if (driftErr)
            logEvent('warn', 'drift.persist.fail', { file: fileNow, error: driftErr });
        if (doc.file !== fileNow) {
            logEvent('error', 'persist.doc-stolen', { site: site || '', expected: fileNow, actual: doc.file });
            return null;
        }
...
async function loadInto(name, create, target, policy) {
    ...
    if (text !== null) {
        ...
        if (create) {
            ...
            var inheritedText = null;
            if (fs) {
                try {
                    var t2 = await fs.resolve(doc.file);
                    var i2 = await fs.stat(t2);
                    if (i2)
                        inheritedText = await fs.readText(t2);
                }
                catch (e2) {
                    inheritedText = null;
                }
...
function projectKeyOfTarget(target) {
    ...
    if (key && rk && (key === rk || key.indexOf(rk + '/') === 0))
        return rk;
    return key;
}
/**
...
function saveActiveSlot() {
    ...
    if (keys.length > DOC_SLOT_MAX) {
        ...
        if (oldest !== activeSlotKey())
            delete docSlots[oldest];
    }
}
 ...
 */
function syncWorkspaceFor(where) {
...
function restoreModel(saved) {
    ...
    if (typeof m.revision === 'number')
        doc.revision = m.revision;
    if (typeof m.updatedBy === 'string')
        doc.updatedBy = m.updatedBy;
...
function applyOps(ops) {
    ...
    for (var i = 0; i < ops.length; i++) {
        ...
        if (kind === 'mark_note') {
            ...
            if (!mnNode) {
                problems.push(tag + ': 找不到节点 ' + (mnId || '(空)') + '，mark_note 需要有效的 id');
                continue;
            }
            if (!mnNode.note) {
                problems.push(tag + ': 节点 ' + mnId + ' 上没有留言，无需标记');
                continue;
            }
        ...
        if (kind === 'add_node') {
            ...
            if (op.shape !== undefined && !SHAPE_WRAP[op.shape]) {
                problems.push(tag + ': 未知形状 ' + String(op.shape));
                continue;
            }
            if (op.x !== undefined && (typeof op.x !== 'number' || !isFinite(op.x))) {
                problems.push(tag + ': x 必须是有限数字');
                continue;
            }
            if (op.y !== undefined && (typeof op.y !== 'number' || !isFinite(op.y))) {
                problems.push(tag + ': y 必须是有限数字');
                continue;
            }
            ...
            if (ngid) {
                var ngHave = false;
                for (var ngi = 0; ngi < doc.groups.length; ngi++)
                    if (doc.groups[ngi].id === ngid) {
                        ngHave = true;
                        break;
                    }
                if (!ngHave)
                    doc.groups.push({ id: ngid, label: String(op.group) });
            }
            doc.nodes.push({
                id: nid,
            ...
            if (typeof op.link !== 'string') {
                problems.push(tag + ': link 必须是字符串（清空传空串）');
                continue;
            }
            ...
            if (!hasEdge(lf, lt)) {
                problems.push(tag + ': 这两点之间没有连线（要新建用 add_edge）');
                continue;
            }
            ...
            if (typeof op.group !== 'string') {
                problems.push(tag + ': group 必须是字符串（移出分组传空串）');
                continue;
            }
            ...
            if (rgid === null)
                continue;
            var rgExists = false;
            for (var rgi = 0; rgi < doc.groups.length; rgi++)
                if (doc.groups[rgi].id === rgid) {
                    rgExists = true;
                    break;
                }
            if (!rgExists) {
                problems.push(tag + ': 找不到分组 ' + rgid);
                continue;
            }
            ...
            if (!Array.isArray(op.files)) {
                problems.push(tag + ': files 必须是数组（清空传空数组）');
                continue;
            }
            var nextFiles = [];
            var rawFs = op.files;
...
var noteStoreCache = {};
var noteStoreLoading = {};
function noteStorePathFor(file) {
    var s = String(file == null ? '' : file).replace(/\\/g, '/');
    var i = s.lastIndexOf('/');
    var dir = i >= 0 ? s.slice(0, i) : '.';
    return dir + '/notes.json';
}
function noteKeyFor(file) {
    var s = String(file == null ? '' : file).replace(/\\/g, '/');
    var i = s.lastIndexOf('/');
    return i >= 0 ? s.slice(i + 1) : s;
}
async function loadNoteStoreFor(file) {
    var storePath = noteStorePathFor(file);
    if (noteStoreCache[storePath])
        return noteStoreCache[storePath];
    if (noteStoreLoading[storePath])
...
function applyNoteStore(file, nodes, legacyNotes) {
    if (!file || !Array.isArray(nodes))
        return;
    var storePath = noteStorePathFor(file);
...
function harvestNoteStore(file, nodes) {
    if (!file || !Array.isArray(nodes))
        return;
    var storePath = noteStorePathFor(file);
    ...
    for (var k in diagramNotes) {
        if (diagramNotes[k] && diagramNotes[k].done === true)
            doneKeys.push(k);
    }
    if (doneKeys.length > NOTE_HISTORY_MAX) {
        ...
        for (var d = 0; d < drop; d++) {
            ...
            for (var ni = 0; ni < nodes.length; ni++) {
                if (nodes[ni] && nodes[ni].id === goneId) {
                    nodes[ni].note = '';
                    nodes[ni].noteDone = false;
                }
            }
        }
    }
 ...
 * 这类闸门复制第二份，早晚会有一份忘了改。
 */
async function writeSidecarJson(storePath, store, policy, tag) {
...
async function saveNoteStoreFor(file, policy) {
    if (!file)
        return null;
    if (!fs)
        return 'fs 服务不可用';
 ...
 */
async function renameNoteStoreFor(fromFile, toFile, policy) {
    ...
    if (!store || typeof store !== 'object')
        return null;
    if (!store[fromKey])
        return null; // 这张图本来就没留言 —— 不写盘，别凭空造一个空表出来
    if (!store[toKey]) {
        ...
        if (!dst || typeof dst !== 'object' || Array.isArray(dst)) {
            ...
            for (var nid in src) {
                if (Object.prototype.hasOwnProperty.call(src, nid))
                    dst[nid] = src[nid];
            }
        }
...
var DRIFT_BUILD_DIRS = { lib: 1, es: 1, cjs: 1, esm: 1, umd: 1 };
var driftStoreCache = {};
var driftStoreLoading = {};
// 「这张表这一轮读失败了」的标记：读失败与「文件本来就不存在（合法空表）」是两件事，
// 后者可以照常写新基线，前者再写就是拿一张空表去盖别人的基线。
var driftStoreLoadFailed = {};
var driftWalkCache = { root: '', at: 0, dirs: {}, files: 0, truncated: false };
function driftStorePathFor(file) {
    var s = String(file == null ? '' : file).replace(/\\/g, '/');
    var i = s.lastIndexOf('/');
    var dir = i >= 0 ? s.slice(0, i) : '.';
    return dir + '/' + DRIFT_FILE;
}
function driftKeyFor(file) {
    var s = String(file == null ? '' : file).replace(/\\/g, '/');
    var i = s.lastIndexOf('/');
    return i >= 0 ? s.slice(i + 1) : s;
}
async function loadDriftStoreFor(file) {
    var storePath = driftStorePathFor(file);
    if (driftStoreCache[storePath])
        return driftStoreCache[storePath];
    if (driftStoreLoading[storePath])
...
async function fpOfRef(ref, root) {
    if (!fs)
        return '';
    var parts = splitFileRef(ref);
    if (!parts.path)
    ...
    try {
        ...
        if (!info)
            return '';
        if (typeof info.size === 'number' && info.size > DRIFT_MAX_BYTES)
            return '';
        var text = await fs.readText(t);
 ...
 */
async function saveDriftStampsFor(docArg, policy) {
    if (!fs || !docArg || !docArg.file)
        return null;
    var root = fileRefRoot();
    ...
    if (!store || typeof store !== 'object' || Array.isArray(store) || driftStoreLoadFailed[storePath]) {
        try {
            store = await loadDriftStoreFor(docArg.file);
        }
        catch (e) {
            store = null;
        }
...
async function sourceDirsOf(root) {
    var now = Date.now();
    if (driftWalkCache.root === root && now - driftWalkCache.at < DRIFT_WALK_TTL_MS)
        return driftWalkCache;
    var out = { root: root, at: now, dirs: {}, files: 0, truncated: false };
    var budget = { dirs: 0, files: 0 };
    async function walk(absDir, rel) {
        if (out.truncated)
            return;
        budget.dirs += 1;
        if (budget.dirs > DRIFT_WALK_MAX_DIRS) {
            out.truncated = true;
            return;
        }
        var kids = [];
        ...
        for (var hs = 0; hs < kids.length; hs++) {
            if (kids[hs].type === 'directory' && kids[hs].name === 'src') {
                hasSrc = true;
                break;
            }
        }
        for (var i = 0; i < kids.length; i++) {
            ...
            var childRel = rel ? rel + '/' + kid.name : kid.name;
            if (kid.type === 'directory') {
                if (SKIP_DIRS[kid.name] || DRIFT_SKIP_DIRS[kid.name])
                    continue;
                if (hasSrc && DRIFT_BUILD_DIRS[kid.name])
                    continue;
            ...
            if (kid.type !== 'file')
                continue;
            if (!DRIFT_SOURCE_RE.test(kid.name))
                continue;
            // 类型声明不是「要画进框架图的源码」：`.d.ts` 只是声明，画它没有意义。
            if (/\.d\.[cm]?ts$/i.test(kid.name))
                continue;
            if (/\.min\./i.test(kid.name))
                continue;
            if (budget.files >= DRIFT_WALK_MAX_FILES) {
                out.truncated = true;
                return;
            }
            budget.files += 1;
...
async function computeDrift() {
    ...
    if (base) {
        var now = {};
        for (var i = 0; i < refs.length; i++)
            now[refs[i]] = await fpOfRef(refs[i], root);
        for (var n = 0; n < doc.nodes.length; n++) {
            ...
            for (var j = 0; j < list.length; j++) {
                ...
                if (!ref)
                    continue;
                if (status[ref] && status[ref] !== 'ok')
                    continue; // 坏掉的由 fileStatus 报，不重复
                var was = base[ref];
                var is = now[ref];
                if (!was || !is)
                    continue; // 没基线、或这次算不出来 → 不猜
                if (was !== is)
                    out.stale.push({ node: node.id, ref: ref });
            }
        }
    }
    ...
    if (!out.external && root) {
        ...
        for (var d = 0; d < dirs.length; d++) {
            ...
            if (underShown)
                continue;
            shown.push(dir);
            if (shown.length > DRIFT_MAX_DIRS_SHOWN)
                break;
        }
        for (var k = 0; k < shown.length; k++) {
            var count = 0;
            for (var kk in walked.dirs) {
                if (kk === shown[k] || kk.indexOf(shown[k] + '/') === 0)
                    count += walked.dirs[kk];
            }
 ...
 */
function promptSafe(s) {
    var out = String(s == null ? '' : s);
    for (var i = 0; i < 8 && out.indexOf('{{') >= 0; i++)
        out = out.split('{{').join('{ {');
    return out;
}
/**
 ...
 */
function foreignCanvasText(sessWhere) {
...
function promptText(asctx) {
    ...
    if (typeof sessWhere === 'string' && sessWhere) {
        if (!docBelongsTo(sessWhere)) {
            ...
            // 目标明确、也不会把中间态写进别人的槽。
            if (!syncWorkspaceFor(sessWhere))
                return promptSafe(foreignCanvasText(sessWhere));
        }
    }
    ...
    if (openNotes.length > 0) {
        ...
        for (var dn = 0; dn < listed; dn++)
            openNotes[dn].noteDone = true;
    }
    if (nc.done > 0) {
    ...
    if (drift && Array.isArray(drift.stale)) {
        for (var ds = 0; ds < drift.stale.length; ds++)
            staleRefs[drift.stale[ds].ref] = 1;
    }
    var refLines = [];
    for (var ri = 0; ri < doc.nodes.length; ri++) {
        ...
        for (var rj = 0; rj < rfs.length; rj++) {
            var rsc = rst[rfs[rj]];
            if (rsc === 'ok' && !staleRefs[rfs[rj]])
                good.push('`' + rfs[rj] + '`');
            else if (rsc === 'ok')
                bad.push('`' + rfs[rj] + '`（文件在图之后改过）');
            else
    ...
    if (drift && (drift.stale.length > 0 || drift.uncovered.length > 0)) {
        ...
        if (drift.stale.length > 0) {
            var top = [];
            for (var dt = 0; dt < drift.stale.length && dt < 5; dt++)
                top.push('`' + drift.stale[dt].ref + '`');
...
ctx.effect(function () {
    return onRpc('doc:applyText', async function (args) {
        ...
        if (!args || typeof args.text !== 'string')
            return { ok: false, error: '需要 text（要清空整张图就显式传空串）' };
        var text = args.text;
        var expectFile = doc.file;
...
ctx.effect(function () {
    return onRpc('doc:rename', async function (args) {
        ...
        try {
            ...
            if (noteMoveErr)
                logEvent('warn', 'notes.rename.fail', { from: a.name, to: b.name, error: noteMoveErr });
        }
        catch (e) {
    ...
    return onRpc('doc:delete', async function (args) {
        ...
        if (doc.name === name && lib.dir === dk.dir) {
            ...
            try {
                ...
                for (var dl = 0; dl < all.length; dl++)
                    if (!all[dl].deleted)
                        lives.push(all[dl].name);
            }
            catch (eLives) {
                lives = [];
            }
...
ctx.effect(function () {
    return onRpc('doc:restore', async function (args) {
        ...
        if (typeof rkRaw !== 'string' || rkRaw.trim() === '')
            return { ok: false, error: '需要 key（图名，或 `子项目/图名`）' };
...
try {
    if (harness && typeof harness.describeRoutes === 'function') {
        var extraRoutes = harness.describeRoutes() || [];
        for (var mr = 0; mr < extraRoutes.length; mr++) {
            if (mountRoutePaths.indexOf(extraRoutes[mr]) < 0)
                mountRoutePaths.push(extraRoutes[mr]);
        }
    }
}
