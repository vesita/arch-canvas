...
function serializeDoc(doc) {
    ...
    for (var ft = 0; ft < seq.length; ft++) {
        ...
        for (var fr = 0; fr < frefs.length; fr++) {
            if (frefs[fr])
                out.push('%% @file ' + seq[ft].id + ' ' + qRef(frefs[fr]));
        }
    }
...
async function persist(policy, site) {
    ...
    try {
        ...
        if (driftErr)
            logEvent('warn', 'drift.persist.fail', { file: doc.file, error: driftErr });
...
function promptText(asctx) {
    ...
    if (typeof sessWhere === 'string' && sessWhere) {
        if (!docBelongsTo(sessWhere)) {
            ...
            if (!syncWorkspaceFor(sessWhere))
                return foreignCanvasText(sessWhere);
        }
    }
    ...
    if (openNotes.length > 0) {
        ...
        if (openNotes.length > 20)
            head.push('- …还有 ' + (openNotes.length - 20) + ' 条未解决的留言，完整内容见文件。');
        // **送达**：把这一批在内存里标成已办。放在最后，保证上面那段清单已经拼进去了。
        for (var dn = 0; dn < openNotes.length; dn++)
