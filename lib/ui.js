...
function ArchStudio(props) {
    ...
    if (model) {
        ...
        for (var ei = 0; ei < model.edges.length; ei++) {
            ...
            pairSeen[ek] = eidx + 1;
            var geo = edgeGeometry(visGeom(ed.from), visGeom(ed.to), visList, en > 1 ? (eidx - (en - 1) / 2) * 10 : 0, { a: portSlots[ei + ':0'], b: portSlots[ei + ':1'] });
            if (!geo)
                continue;
            var dashed = ed.arrow === '-.->';
...
function PendingNotesDock(props) {
    ...
    var nodes = Array.isArray(studioLiveNodes) ? studioLiveNodes : [];
    for (var i = 0; i < nodes.length; i++) {
        if (nodes[i] && nodes[i].id && nodes[i].note && !nodes[i].noteDone)
            pending.push(nodes[i].id);
    }
    if (pending.length === 0)
        return null;
    ...
    for (var k = 0; k < pending.length; k++) {
        if (String(draft || '').indexOf('@' + pending[k]) < 0)
            missing.push(pending[k]);
    }
    ...
    return React.createElement('div', { className: 'ac-pending' }, React.createElement('span', { className: 'ac-pending-n' }, '留言 ' + pending.length + ' 条待发' +
        (allIn ? '（已全部在输入框中）' : (draft.trim() ? '（只补还没放进来的，追加在后面）' : ''))), React.createElement('button', {
