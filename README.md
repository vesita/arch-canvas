...
| 词 | 指什么 | 代码里的对应物 |
|---|---|---|
| **源文本** | 那份 Mermaid 文本 —— 唯一真相。坐标、锚点都寄生在它的 `%%` 行里，所以它到哪都是合法 Mermaid | `<图名>.mmd` 的内容 |
...
以前你说「第二个方框不对」，全靠 AI 去猜是哪个节点。现在可以**把留言写在元素上**：
选中节点 → 检查器里写留言，节点左上角出现琥珀色笔角标。留言以旁路表存储在图同目录的 `notes.json` 中：
`{ "<图文件名>": { "<节点id>": { "text": "...", "done": false } } }`

- **源文本纯净**：源文本里没有留言行，图体与源文件保持干净；
- **指代消失了**：话挂在节点 id 上，AI 不用再猜你指的是哪个；
- **一次投递**（2026-09-21 改）：留言从「待办」变成「消息」—— AI 读到一次就消耗掉，不会反复出现，也没有任何状态要你去维护；
- **走读路径**：待递的留言每步进上下文，与「AI 能不能改图」是两件事 —— 这是你和 AI 之间一条单向通道（你写，它读）；
- **只有一段历史**：工具栏「留言 N」待递在前、历史在后（最多留 6 条），点一行定位到那个节点；想让某条重发就点「再送一次」。**长期挂着的意图不要放留言**（投递一次就没了），那应该写进节点描述；
- **删节点不丢留言**：留言独立于源文本，删掉节点后留言留在表里（孤儿保留），不会有注释把节点拽回图里的幽灵问题；
- **发送入口**：留言清单头部「交给 AI (N)」一个动作把整批交出去 —— 输入框空着也行（它替你把这一批写成一条消息再发出去），你已经打了字就原样发你那句。检查器里还有单节点「发送给 AI」。另有聊天输入框 `@` 引用画布节点展开模型文本。
- **不往输入框里塞东西**：未办留言由宿主每一步注入，界面**不再**自动往草稿里补 `@id`（2026-09-21 移除：那和注入在送同一件事，还让「输入框是空的」这个状态永远不存在）。
...
- **`@file`（代码锚点）**：选中节点 → 检查器里填路径（`路径` 或 `路径#符号`，一行一条）。
  ...
  **还有一层它自己看不见的腐烂**：文件还在、符号还在，但那段代码在图上一次写进文件之后被改过了
  （改名、职责搬走、参数换掉）—— 锚点照样「有效」，而内容早就不是图上说的那个东西。
  这一层靠**内容指纹**比出来（落盘时记一遍、之后每次加载比）：角标转琥珀、画布顶部出一条
  「这张图可能已经过期」，提示词里也逐条说明「别照着这些锚点走」。
  同一处还会报**漏画**：哪些目录里有源码、却没有任何锚点指向（它只是「还没画」，语气低一档，不是报错）。
...
| 通道 | 内容 |
|---|---|
| 每步注入 | `systemPrompt.context` 把当前 Mermaid 全文塞进模型上下文（`%%!` 格式说明行滤掉，其余逐字） |
| 一句话总结 | `%% @summary` 单列成「**这张图讲的是**」；同一图库里别的图的总结也一并告知 |
| 元素留言 | 用户写在元素上的留言（存 `notes.json`）单独列成一节：待递的会被 AI 读一次就消耗，历史最多留 6 条 |
| 代码锚点 | `%% @file` 单列成「图元素上标的代码锚点」，逐条带校验结果，失效的带 ⚠ 与「不要照着用」 |
| 保鲜 `drift` | 「图上次写进文件之后，哪些锚点的文件改过了」「哪些目录有源码却没画到」—— 只有非空时才注入 |
| `arch_read` | 读当前图源码（通常不必调，上下文里已有） |
| `arch_edit` | 增量改图，**推荐**；不破坏用户布局 |
| `arch_switch` | 切到另一张图（用户画布跟着切）；不存在时报错并列出可选 key |
| `arch_write` | 整图替换；会继承同 id 节点的旧坐标、**代码锚点**与一句话总结；留言保存在旁路表中按 id 对应 |
...
- **留言只挂在节点上**：连线、分组还不能留留言（连线的 id 是位置派生的 `e1`/`e2`，不稳定）；一个节点只能有一条留言，开不了「讨论串」。代码锚点同理只挂节点。
- **一句话总结只有一句**：`%% @summary` 是单行、500 字上限，写不了多段「背景 / 结论 / 待定」。
- **代码锚点靠字符串找符号**：`路径#符号` 的「符号存在」是一次 `indexOf` 全文搜索，
  不区分「定义」与「引用」—— 换名重构过的文件可能假阳性。
- **保鲜（drift）只认「变没变」，不认「变得对不对」**：指纹变了就提醒，改一行注释也会算
  「改过」；而且基线是「图上一次落盘」那一刻 —— 你如果一直在改代码却从没动过图，
  这条提醒就该一直在（那正是它要说的）。指纹表 `.arch-canvas/anchors.json` 是派生数据，
  删掉不会有任何损失，只是重新攒一次基线。
- **漏画只看目录，不看文件**：报的是「哪个目录里的源码一条锚点都没有」，不是逐个文件列。
- **连线是正交折线，但没有真正的路由器**：平行线会互相让开车道、十字交叉处拱一下，可它仍然是
  「按端口偏移 + 几条固定车道」算出来的，不是 A* 寻路 —— 方块之间挤得很密时，线仍可能贴着别的方块走。
- **自动布局只是起点**：简单分层，交叉多了不如 dagre。
- **撤销历史只在内存里**：上限 60 步，**切图时按设计清空**，重启就没了（要跨会话回退用「历史」面板里的检查点）。
  切到「对话」页再切回来**不会**再弄丢它 —— 视角与撤销栈会按「会话 + 图」记在内存里还回来。
- **空白会话里没有入口**：画布是主窗口的一个子页，「对话」这张皮只在你有一个会话时才渲染，
  所以还没有任何对话时，找不到地方打开画布。
- **`@` 候选按会话取**：某个会话里从没打开过画布时，`@` 菜单里不会列出节点（打开一次就有了）。
  从前它会拿「上一次打开的那张图」顶上 —— 那是跨项目的错数据，宁可空着。
