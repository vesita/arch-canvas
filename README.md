...
- **跨轮持久**：聊天会被截断、会跑题，未办留言每步都在（随上下文注入）；
- **走读路径**：未办留言每步进上下文，与「AI 能不能改图」是两件事 —— 这是你和 AI 之间一条单向通道（你写，它读）；
- **有生命周期**：工具栏「留言 N」列出未办 / 已办两区，点一行定位到那个节点；标记「已办」之后就不再注入 —— 避免留言单调累积干扰 AI 重新讨论定案；
...
| 元素留言 | 用户写在元素上的留言（存 `notes.json`）单独列成一节未办清单（已办的不注入，只报条数） |
