...
1. **对用户一律叫「留言」，不叫「注释」；代码字段保持 `note` / `noteDone`。**
   它是**一次投递**（不是待办，见第 3 条），存与那张 `.mmd` 同目录的 `notes.json` 旁路表：
   `{ "<图文件名>": { "<节点id>": { "text": "...", "done": false, "at": 0 } } }`。
...
3. **留言是一次性消耗的**（2026-09-21 定，用户原话「传输给一方后就移除，最多留一个历史查询」）。
   `promptText()` 读到那一段清单就算**送达**，随即在内存里把这一批标成已办 —— 此后它不会再出现在
   任何一步的上下文里，也不会回到清单。**僵尸驻守于是从结构上消失**，不需要任何规矩去防。
   （走到这一步之前试过两版：让用户去点、让 AI 自己标 —— 都是在维护一个本可以不存在的状态。）
   - 提示词里明说「这几条本轮之后就会从你的上下文里消失」，逼 AI **本轮一次处理完**。
   - **历史**：`done` 的就是历史。落盘那一刻（`harvestNoteStore`）按 `at`（进入历史的时刻）丢最旧的，
     只留 `NOTE_HISTORY_MAX = 6` 条。`at` **不在模型上**，是落盘时才盖的 —— 所以不用碰
     `normalizeModel` 白名单 / `snapshotModel` / `restoreModel`（少碰三处就少三个静默丢字段的机会）。
   - **这是一条读路径上的内存变更**：`promptText` 只改内存，落盘照旧由 `persist()` 统一做。
     代价写在注释里：送达之后如果一直没人写盘，重启 dsh 会让这批**再送一次** ——
     宁可多送一次，也不丢用户写的东西（与「检查点：安全靠退得回去」同一个取向）。
   - 留言的**正文**只由用户写：AI 能改的只有状态（`mark_note`，界面拿它做「再送一次」）。
     **要长期挂着的意图不要放留言**（投递一次就没了）—— 那是节点描述或图内容的活。
   守门人：`test/host.e2e.mjs`【留言历史封顶 6 条】+【提示词注入】里的
   「送达之后同一条不再出现在提示词里」（后者就是「僵尸没了」本身）。
...
7. **客户端发送入口与 `@` 引用源**：
   - 清单头部「交给 AI (N)」**一个动作把整批交出去**：输入框空着就替用户把这一批写成一条消息
     （画布名 + 逐条 id/标题/正文）再 `submit()`；输入框里已经有用户打的字，就**原样发他那句**、
     一个字都不动。检查器单节点仍提供「发送给 AI」（那条只填入，不自动发）。
   - **未办留言不再自动补进输入框**（2026-09-21 移除）。理由：`promptText()` 每一步已经把未办留言
     连正文一起注入了，那串 `@id` 是第二遍搬运；代价却是实的 —— 它让「输入框是空的」这个状态
     根本不存在，于是「空框也能发」这条永远测不出来。`draftHasRef` 随它一起删掉了。
   - **空草稿发不出去是 DSH 的规矩，不是我们的选择**：输入框的 Enter 通道明确判 `!empty`，
     而 `inputActions.submit()` 落到同一台状态机（空草稿 + 无附件是文档里写明的 no-op）。
     所以「空框也能发」只能由我们补一句话达成 —— 别指望 `submit()` 自己发得出一条空消息。
   - 两条路都走客户端 `props.inputActions`（`setDraft` / `submit`；DSH 提交管线，必须判空）。宿主半边
     禁止 import，拿不到 `createUserMessage`，走客户端避免手搓脆弱的消息结构；
...
- **跳过名单**：与图库扫描共用 `SKIP_DIRS`，再补 `DRIFT_SKIP_DIRS`（test / fixtures / generated /
  vendor / assets / docs…），并跳过 `*.min.*` 与 `*.d.ts`（类型声明不是「要画进框架图的源码」）。
  再加一条**只在同级真有 `src` 时**生效的：`lib` / `es` / `cjs` 是编译产物
  （`DRIFT_BUILD_DIRS`）—— 这一条是拿本仓库自己试出来的：第一次跑 drift，`lib/` 里 4 个产物文件
  被报成「有源码没画到」。别把它做成无条件跳过：别的项目把真源码放 `lib/` 是合法的。
...
**继承全局图库只在目标图库还是空的时候发生**（`inheritGlobalOnce`）：判据是「目标图库里有没有活着的图」，
不是任何标记文件；而且逐张复制时**同名一律跳过**。这两道护栏都来自一次真实事故（2026-09-21）：
从前拿「全局目录下的 `.inherited`」当「只做一次」的标记，而那个标记与**目标**无关 ——
一个从没走到过继承的项目，`arch_switch { create: true }` 会把全局兜底图库整份盖到它身上，
**同名图被静默覆盖**（本仓库的 `architecture.mmd`，23 个节点，就是这么没的；日志里一条都不留，
因为整条路径只往 `doc.warnings` 里塞一句话）。修完的护栏顺序是：目标有活图 → 直接返回；
否则逐张复制，目标已有同名文件 → 跳过；一个字节都不往会话工作区之外写。
守门人：`test/host.e2e.mjs`【回归：建新图不许覆盖已有的图】
（已有的图一个字节不动 / 新图照建 / 墓碑文件也不被覆盖 / 第二次进同一项目不再继承）。
...
## 画布交互三件：拖动 / 详情面板 / 连线锚定（2026-09-23）

用户报的三处「手感」问题，先调研成熟方案（draw.io / JointJS / React Flow / Excalidraw /
tldraw / VS Code / ELK）再动手。三条结论都是**先实测、后实现**，数字留在守门人旁边。

### 1. 拖动：一帧一次更新，浮点跟手，吸附按屏幕像素

- **rAF 合帧**（`rafFrame` / `rafCancel`，`runtime.ts`）：`pointermove` 只写 pending，一帧最多
  一次 `setLocal`。**抬手前必须 flush**（`flushMove`）——否则最后一段位移会丢；而且
  「点空白还是平移」正是由最终位移决定的，不 flush 会把一次平移误判成点空白、连选中一起清掉
  （这条被 [4w] 当场抓到过）。平移也走同一条路。
- **拖动期用浮点坐标，提交时才取整**（`commitDrag`）：每帧 `Math.round` 会把位移量化成
  k 像素的台阶（放大时肉眼可见）。宿主落盘本来就会取整（`mermaid.ts` / `normalizeModel`），
  所以跟手不会换来「文件里出现小数」。
- **吸附阈值按屏幕像素折算**（`snapRadius(k, SNAP_PX)` = 8/k，钳在 2~28 模型单位）：
  旧值是模型单位 8 —— 缩到 0.5 倍时屏幕上只剩 4px（吸不住），放到 2 倍时是 16px（甩不开）。
  **迟滞** 8px 吸上 / 14px 脱开（`snapAxis` 的 active 分支），**Alt 临时关掉**。
  分组拖动走同一条吸附（从前只有节点吸，同一个动作两种手感）。
- **pointercancel = 回滚**（`restoreDrag`，不写盘不进历史）；**lostpointercapture = 提交** ——
  用户已经在屏幕上拖出来了，静默丢弃是「安全靠退得回去」的反面。
- `DRAG_SLOP = 4` 是「点选 vs 拖动」的门槛，和面板的展开挂钩（见下），**不要**为了手感去动它。

### 2. 详情面板：收起 ≠ 关闭（三态）

- 状态是**三态**：展开 / 收起成一条细条 `.ac-peek`（选中保留）/ 无选中。收起只收面板，
  关闭（`×`）才取消选中；点**同一个元素**再点一次 = 收起（`toggleDockFor` + `dockForRef`，
  走 ref 是因为「down/up 之间没有重渲染」时也要成立）。
- **Esc 分层**：焦点在输入框 → 只退焦点；否则面板开着 → 只收起（保留选中）；再一下才清选中。
- **点空白只在「无位移的抬手」上清选中**（`onBackgroundDown` 不再动选中）—— 想挪视角的人
  不该丢掉正在编辑的节点。
- 面板开合会改变 stage 高度：`keepSelInView()` 把选中节点**最小平移**带回视野（不重新 fitView，
  用户摆的视角不能被一次开面板冲掉）；ResizeObserver 在适应过窗口之后也走这条。
- 面板高度由 `.ac-sash` 拖出来，**只活在模块变量 `STUDIO_DOCK_H` 里**（不落盘，与视角/撤销栈
  同一条记忆规则）。标题栏不滚动，只有 `.ac-dock-body` 滚。

### 3. 连线锚定：先落点、再定侧；落点必须在**真实轮廓**上

- 选边从「|dy| >= |dx| 就上下」改成 `edgeSidesOf`：两端**同轴**的两种组合里挑代价最小的
  （代价 = 边心曼哈顿距离 + 「一条轴开着却从另一条重叠轴绕出去」的罚 + 回头线/自穿透重罚）。
  实测 420 个相对位置：旧规则平均比同轴最优多绕 24.8px、26.7% 的位置多绕 40px 以上；
  新规则 3.4px / 0%。**不做** 16 组合枚举（要混轴 L 形布线器）、**不抄** mxGraph 的
  routePatterns 查表（4 象限 × 16 模板绑死 jetty 语义）、**不用** ELK（整图重排会毁掉用户坐标）。
  选边还带一条 `axisLooksClear`（给 obstacles 时才算）：一条轴的中位线附近全被挡住就加罚 ——
  没有它的话，选出来的轴会被障碍全部拒掉、落到硬穿兜底上（真图实测过）。
- **落点投到轮廓**（`perimeterPoint`）：矩形/胶囊/椭圆/菱形/六边形各一条闭式解。
  从前一律落在包围盒上 —— 同一侧挤两根线时箭头悬在方块外（菱形 24.4px、椭圆 10.1px、
  六边形 5.4px）。三条实现注意：
  1. **胶囊的圆角圆心离形状中心有 (spanD - r) 的偏移**，少了它横着出去的锚点会缩进方块里
     （140×60 上实测缩 41px）；
  2. 六边形的 14px 内缩是 `NODE_HEX_INSET`，**画形状与算锚点共用同一个常量**；
  3. **「有没有穿进自己」不能用包围盒判**：非矩形节点的锚点本来就落在包围盒内部（轮廓上），
     拿包围盒当判据会把每条候选都判成自穿透，整条线于是落到不看障碍的硬穿兜底上 ——
     画出来正好穿过别的方块（复核抓到的高危项，8 种形状 × 2 端口全中）。现在走
     `edgePathHitsSelf` → `edgeSegHitsNode`：包围盒只当快速预筛，再用 `shapeInterior` 按
     形状采样判「真的进去了」，于是「从轮廓上出发往外走」天然不算。
- **兜底绕行的车道要铺开**：`兜底一`（两段绕行）从前只试「整片障碍外侧的左右两条车道」，
  方块挤成一片时那两条常常紧贴别的方块、一条都走不通，于是落到**不看障碍的硬穿**。
  现在把每块障碍的外侧也当候选（按离中位线远近排）。实测 `dsh-plugin-framework`：
  穿过第三方块的连线从 5 条（改前）→ 7 条（选边变聪明之后）→ **0 条**，总折线还短了 9%。
- **端口分组（studio）与布线器（runtime）必须问同一个 `edgeSidesOf`**：从前是同一规则的两份
  拷贝，选边一旦变聪明，两份必然分叉（端口按 A 侧均分、线却从 B 侧出去）。
  `gstate` 因此要把 `shape` 一起给连线用；折叠块没有 shape，按矩形处理。
- 锚点的守门断言用**有符号**距离（0 = 在轮廓上，负 = 埋在形状里）：
  只测「离轮廓多远」的话，锚点缩到方块正中间也会算 0 —— 那条断言就没牙了。

守门人：`test/edges.test.cjs`（35 条：轮廓有符号距离 × 两种摆放 × 8 种形状 × 1~4 端口、
选边三条、420 点网格的「白绕/回头线/从背后出线」不变量 + **旧规则负向对照**、
非矩形多端口「不穿别人也不穿自己」128 例（含采样判据的负向对照）、平行间隔让道 + 负向对照、
避障、自环、坐标完全重合的退化输入）；
`test/ui.render.mjs` 第 [4r] G（面板三态 / Esc 分层 / 分隔条，11 条新增）、第 [4p] 平行间隔
（坐标是**穷举搜出来**的：两条都是干净 Z、中位线相同、跨度重叠，否则断言空转）与第 [4w] 节
（rAF 合帧不丢位移、吸附的屏幕阈值/迟滞/Alt、pointercancel 回滚、lostpointercapture 提交、
分组吸附；缩放从画布 transform 读，不抄滚轮步长）。
真机冒烟：`.arch-canvas/architecture.mmd`（24 节点 27 连线）与 `dsh-plugin-framework.mmd`
（28 节点 35 连线）上都是 0 条 null、**0 条穿过第三方块**（后者改前是 5 条）。

## 画布属于项目，不属于进程（2026-09-23 跨项目丢留言事故）

**事故**：另一个会话（另一个项目）把共享画布切到它自己的图之后，本会话每一步的提示词注入都读到了
那张图；更糟的是，留在那上面的 3 条留言被本会话的步骤当成「自己的、读一次即送达」消费掉了 ——
用户写的东西投给了错的会话。根因只有一条：**宿主只有一份内存文档，而 `promptText` 是同步求值、
拿不到会话**，于是「当前文档是谁的」这件事在注入路径上根本不存在。

四条不许破：

1. **`promptText(asctx)` 必须按会话判归属。** DSH 会把装配上下文（`{ agent, scope, signal }`，
   见 `dsh-agent` 的 `assembleContextFor`）传给注册的 `text` 函数 —— 里面的
   `agent.session.cwd` 就是这一步所属的项目。**画布不属于你（`docBelongsTo(where)` 为假）时**：
   - 先试着同步换到自己项目那一份（内存槽命中，`syncWorkspaceFor`）；
   - 命中不了就只注入一段**说明**（`foreignCanvasText`）：画布停在哪个项目、为什么这一轮看不到内容、
     要看自己的图去调 `arch_read`。**不含任何别人的图内容、锚点、留言**，也不消费那些留言。
   - 没有会话信息（工具、测试桩、headless）时**保持旧行为** —— 别把「判不了」变成「一律不给」。
2. **留言只投递给它所属项目的会话。** 这是第 1 条的推论，但值得单列：留言是**一次性**的投递，
   投错人等于丢了。守门断言就是「Y 那一步没吃掉 X 的留言」+ 盘上 `done` 仍为 false。
3. **每个项目一份内存画布（`docSlots`）**，槽里放的是**同一批对象引用**，切换只是换指针：
   - 换项目时 `saveActiveSlot()` → `activateSlot(key)` 命中就返回（**不读盘、不 bump 修订号**，
     上一步投递过的留言状态也原样留着）；
   - **只存「已经载入过」的状态**（`loadedFor` 为空时不存）：换项目会先重置成一份空白文档再去加载，
     把那个中间态存进去，下次「命中」就等于命中一张空图（实测踩到过）；
   - **换新项目必须给一份全新的 doc 对象**（`newDocState()`）：原地改会把上一个项目槽里那份一起改掉
     （实测：X 的槽变成 Y 的内容，回到 X 就等于丢了 X 的图）；
   - 归属的键是**项目根**（`projectKeyOfTarget`），不是子图库那一层 —— 否则用户一下钻子图库，
     自己的提示词就被判成「别人的画布」。
4. **读路径不发起加载。** `promptText` 里**不许** `ensureLoaded(...)`（早先顺手排过一次异步加载：
   它与别的会话的切库抢同一个活动指针，把一份空文档写进了某个项目的槽）。要画布走 `arch_read` ——
   那条路带着会话 cwd，目标明确。

另外两条实现上的坑：

- **`doc.workspace` 必须留在 `RUNTIME_ONLY_FIELDS`**（mermaid.ts）。它不进文件，漏了就是
  **每次保存都误报** `serialize.not-idempotent` —— 这次加会话隔离当场踩到，就是那条守门人逮住的。
- **残留（未修）**：`whereOfExec` 取不到 cwd 的会话（无 cwd 的 headless）仍按「当前文档」读写。
  这是既有行为，不是这次引入的；真撞上再收。

守门人：`test/host.e2e.mjs`【会话隔离：画布属于项目、不属于进程】22 条 ——
Y 那一步读不到 X 的图/留言/源文本块、盘上 `done` 仍为 false、X 那一步仍能拿到自己的留言、
一次性投递语义不变、工具带回自己的画布、换回 X 修订号不动（附「第一次进新项目会标 switch」的
负向对照）、子图库归属仍按项目根、无会话信息时保持旧行为。
