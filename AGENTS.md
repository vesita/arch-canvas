...
3. **注入语义：只有未办留言进 `promptText()`。**
   ...
   **AI 处理完要自己 `mark_note` 标掉，不许留给用户去点**（2026-09-21 翻转的规矩）。
   从前写的是「处理完提醒用户去界面标已解决」，那条规矩造出**僵尸驻守**：用户不会为了这件事专门
   回去点，于是同一段清单每一步原样重来 —— AI 反复处理同一件事，还一直占着上下文。
   现在与「检查点回退」同一个思路：默认往前走，纠错留一条便宜的后路（用户在清单里一键「重开」），
   而不是把每件事都先问一遍。护栏：**没做的不许标** —— 标了它就从 AI 眼前消失，用户却还等着。
   留言的**正文**仍然只由用户写：AI 能改的只有状态。守门人：`test/host.e2e.mjs`【AI 标记留言已办 mark_note】
   （标完**提示词里不再出现那一条** —— 这条断言就是「僵尸没了」本身）。
